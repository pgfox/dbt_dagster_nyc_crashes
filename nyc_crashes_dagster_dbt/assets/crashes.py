...
CSV_PATH = Path("data/Motor_Vehicle_Collisions_-_Crashes.csv")
SCHEMA = "raw"
TABLE = "crashes"
...
def crashes_raw(context: AssetExecutionContext, postgres: PostgresResource):
    # Read header from CSV and normalize column names
    ...
    with postgres.get_cursor() as cur:
        ...
        # Bulk load via COPY FROM STDIN (skip header row already consumed)
        with open(CSV_PATH, newline="") as f:
